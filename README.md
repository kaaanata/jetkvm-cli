...
Coordinate bindings default to 30 seconds from source frame receive time (`captured_at` / `frame.received_at`). Capture freshness is separate: ordinary observations may reuse a fresh continuously decoded frame; post-action observations still require a frame received after input completion. `frame.decoded_at` records decode completion and does not renew the binding. Never restamp metadata; an expired binding requires a new observation.
