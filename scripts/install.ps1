...
try {
    ...
    $checksumMatches = @(Get-Content -LiteralPath $checksumPath | ForEach-Object {
        if ($_ -match '^([0-9a-fA-F]{64})\s+(.+)$' -and $Matches[2] -eq $archiveName) { $Matches[1].ToLowerInvariant() }
    })
    if ($checksumMatches.Count -ne 1) { throw "Release checksum entry is missing, invalid, or not unique for $archiveName." }
    $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $archivePath).Hash.ToLowerInvariant()
    if ($actual -ne $checksumMatches[0]) { throw "SHA-256 verification failed for $archiveName." }
