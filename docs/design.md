...
`key_hold` is an input batch action with `keys` and `duration_ms` (1..12000), also available as `jetkvm input hold <device> <key> --duration 10s`. All holds require confirmation. The lease owns press, bounded keepalive, release and terminal neutralization. Keepalive is HID-RPC type 0x09, not repeated keypresses. Missed keepalive deadlines fail closed. An accepted receipt proves transport delivery, not continuous physical host behavior.
