...
The default coordinate binding lifetime is 30 seconds from the source frame's first RTP receive timestamp. `captured_at` uses this source receive time, also recorded as `frame.received_at`; `frame.decoded_at` separately records decode completion. Decoding, transport to the model, and model reasoning consume the binding lifetime rather than resetting it. Clients must not restamp metadata. An expired binding requires a new observation. Capture freshness is a separate bound: ordinary capture may reuse a decoded frame within the freshness requirement, but never a frame before the session's latest input completion. Observe-after, batch capture, and wake recovery retain server-owned source-time lower bounds.

The default capture freshness bound is 5 seconds for compatibility with cold-device delivery and decoding. Callers can request a stricter bound with CLI `--freshness` or MCP `freshness_ms`; zero selects the service default. This setting does not extend the 30-second coordinate binding lifetime or replace source timestamps with decode completion time.
...
Text confirmation policy when `confirmation.required` is enabled:
...
The archive compatibility contract is exactly four regular root files: `jetkvm` (`jetkvm.exe` on Windows), `LICENSE`, `NOTICE`, and `README.md`. Existing strict updaters and both bootstrap installers enforce this contract. Release preparation appends the complete codec license text to generated `NOTICE`; nested decoder manifests and license sidecars must not become extra archive entries. An explicit C codec inventory supplies `decoder.sbom.json` for vulnerability scanning. Both that SBOM and complete corresponding source/build material in `decoder-source.tar.gz` are independent assets covered by signed checksums and release attestation. Before signing or uploading, the release gate scans all six actual tar/ZIP outputs with the unchanged production extractors, compares extracted binaries byte-for-byte with build outputs, checks full codec attribution, and verifies archive/SBOM checksums and codec inventory.
...
New configurations allow the non-device-scoped `setup` administrative toolset alongside observation/video; existing explicit ceilings are not extended during enrollment. Input requires an explicit choice; secondary action confirmation is optional and defaults off. Settings updates support output defaults, explicit global input enablement, per-device exposure/input/takeover permission, secondary action confirmation, and idle/absolute control lifetimes. They do not mutate stable identity or route bindings, TLS pins, credential sources, state paths, power permissions or administrative policy. Global input and per-device input are separate explicit settings; enabling only a device cannot silently expand a deployment ceiling.
...
## Bounded hold and power-path evidence

MCP action confirmation is an approval-only form with no user-entered properties. Exact target, capabilities, lifetimes, operation arguments and policy remain bound by sealed request state and one-time proof consumption. Host policy decides how approval is obtained; a host decline is not attributed to a human. Input/power approval follows the same contract.

`key_hold` is an input batch action with `keys` and `duration_ms` (1..12000), also available as `jetkvm input hold <device> <key> --duration 10s`. Holds require confirmation when the global switch is enabled. The lease owns press, bounded keepalive, release and terminal neutralization. Keepalive is HID-RPC type 0x09, not repeated keypresses. Missed keepalive deadlines fail closed. An accepted receipt proves transport delivery, not continuous physical host behavior.

`power capabilities` and video-control readback distinguish ATX contact control, USB System Control and Keyboard Power. Current upstream RPC provides ATX durations of 200 and 5000 ms, but the underlying serial driver accepts durations and writes BTN_PWR_ON/OFF. The extension MCU holds its output until OFF; its watchdog is not a disconnect-triggered release timer. A safe configurable physical hold requires a device-owned maximum-duration release and matching hardware wiring. Current serial-console hardware has no proven physical-button path. Keyboard usage 0x66 and vendor-defined USB wake do not establish Apple Silicon Startup Options support.

Operation send ownership is held by an OS file lock before the durable send boundary through terminal receipt commit. Startup recovery skips locked operations and only recovers unlocked send_started rows. Lock paths are shared beside the database and are not unlinked while processes may retain references. Upgrade requires draining pre-lock clients because legacy startup recovery cannot honor the new ownership gate.

Connection establishment is bounded to 20 seconds by default, with metadata/SDP/RPC/HID phase errors. The timeout only owns establishment; successful persistent MCP controls retain their configured idle and absolute lifetimes. No automatic reconnect or replay follows a failed establishment.

## Optional device-action confirmation

Top-level `confirmation.required` defaults to false, including when absent in older configuration files. Authorized CLI/MCP device actions then execute without a second confirmation. Set it through `jetkvm config set --require-confirmation=true --yes` or the revision-bound MCP `confirmation_required` settings patch to enable existing risk-based confirmations. `config show` reports the value. Disabling again uses false. The global switch takes precedence over legacy per-device takeover confirmation flags.

Permissions and takeover allowance are still mandatory, and the setting does not widen a handle's capabilities, bypass generation fences, allow ambiguous retries, or skip neutralization. When enabled, approvals remain argument-bound and single-use. Configuration/integration maintenance approvals are separate from device-action confirmation. The switch participates in the policy revision so outstanding proofs cannot cross a policy change.

### Continuous video reference ownership

The embedded backend is a persistent FFmpeg 9.0.1 H.264-only WASI reactor. It has no external codec or CGo runtime dependency. One session owns the receiver and decoder worker; a bounded ordered AU queue preserves P/B reference dependencies. Loss or overload discards the invalid chain and fences in-flight publication until an independent IDR. Decoder output tokens resolve to original source metadata across reordering. Immutable copied YUV planes allow encoding while subsequent frames decode. Request cancellation ends only observation waiting; session reset and closure cancel and join the stream. See the [decoder decision and evidence](../internal/video/DECODER_DECISION.md).
